...
if mode.split("-")[-1] == 'pip1':
	if mode.split("-")[0] == 'dsciqr':
		path = "inputs-final/stats-v6-pip1-iqr.csv"
...
with open('gbm_results_final.csv', 'a') as f_object:
	writer_object = writer(f_object)
	header = ["Stats file","Mode", "random_id", "n_estimators", "min_samples_leaf", "min_samples_split", "max_depth", "learning_rate", "CI MAE TRAIN", "CI STD TRAIN", "PL MAE TRAIN", "PL STD TRAIN", "CI MAE TEST", "CI STD TEST", "PL MAE TEST", "PL STD TEST", "time needed"]
...
for i in range(1):
	...
	gs = MultiOutputRegressor(ensemble.GradientBoostingRegressor(**params), n_jobs = -1)
	gs.fit(x_train_norm, y_train_norm)

	end = timer()

	pickle.dump(gs, open("pretrained_models/"+f"gbm_{random_id}", 'wb'))
	...
	if mode.split("-")[-1] != 'pip1':
		...
		with open(f'predictions/gbm-predictions-{mode}-{random_id}.txt', 'w') as f_:
	...
	with open(f'predictions/gbm-pred-{mode}-{random_id}.txt', 'w') as f_:
	...
	with open('gbm_results_final.csv', 'a') as f_object:
		...
		row = [path, mode, random_id, e_num, leaf, split, depth, lr, mae_CI_train, std_CI_train, mae_PL_train, std_PL_train, mae_CI_test, std_CI_test, mae_PL_test, std_PL_test,  "{:.2f}".format(float(end-start)/60.0)]
		writer_object.writerow(row)

		f_object.close()

pickle.dump( boxplot_data, open("boxplot_data/boxplot_data_gbm_"+str(random_id)+".p", "wb" ))