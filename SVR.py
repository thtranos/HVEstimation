...
with open('svr_results_final.csv', 'a') as f_object:
	writer_object = writer(f_object)
	header = ["Stats file", "Mode", "random_id", "kernel", "C", "gamma", "epsilon", "tolerance", "CI MAE TRAIN", "CI STD TRAIN", "PL MAE TRAIN", "PL STD TRAIN", "CI MAE TEST", "CI STD TEST", "PL MAE TEST", "PL STD TEST", "time needed"]
...
for i in range(1):
	...
	gs = MultiOutputRegressor(SVR(**params), n_jobs = -1)
	gs.fit(x_train_norm, y_train_norm)

	end = timer()

	pickle.dump(gs, open("pretrained_models/"+f"svr_{random_id}", 'wb'))
	...
	print('MAE CI TRAIN', mae_CI_train)
	print('MAE PL TRAIN', mae_PL_train)
	...
	print('MAE CI TEST', mae_CI_test)
	print('MAE PL TEST', mae_PL_test)
	...
	if mode.split("-")[-1] != 'pip1':
		...
		with open(f'predictions/svr-predictions-{mode}-{random_id}.txt', 'w') as f_:
	...
	with open(f'predictions/svr-pred-{mode}-{random_id}.txt', 'w') as f_:
	...
	with open('svr_results_final.csv', 'a') as f_object:
		...
		row = [path, mode, random_id, kernel, coef, gamma, eps, tol, mae_CI_train, std_CI_train, mae_PL_train, std_PL_train, mae_CI_test, std_CI_test, mae_PL_test, std_PL_test,  "{:.2f}".format(float(end-start)/60.0)]

		writer_object.writerow(row)

		f_object.close()

pickle.dump( boxplot_data, open("boxplot_data/boxplot_data_svr_"+str(random_id)+".p", "wb" ))